 ...
 */
 ...
 void RSA_Encrypt(rsa_pubkey *pubkey,unsigned char *input,unsigned char *output)
{
    rsa_context rsa;
    rsa_fill_publickey(&rsa,pubkey);
    memset(output, '\0', sizeof(output));
    rsa_public( &rsa,input,output);
}
void RSA_Decrypt(rsa_crt_prikey *prikey,unsigned char *input,unsigned char *output)
{
    rsa_context rsa;
    rsa_fill_crt_privatekey(&rsa,prikey);
    memset(output, '\0', sizeof(output));
    rsa_private( &rsa,input,output);
}
int myrand( void *rng_state );
void RSA_Generate_keys(rsa_pubkey *pubkey,rsa_crt_prikey *prikey)
{
    rsa_context rsa;
    rsa_init( &rsa, RSA_PKCS_V15, 0 );
    rsa_gen_key( &rsa, myrand, NULL, 1024, 65537 );
    rsa_getkeypairs(&rsa,prikey,pubkey);
}


int rsa_fill_crt_privatekey(rsa_context *ctx, rsa_crt_prikey *prikey)//put the private key into context
{
    int ret;
    mpi P1,Q1,H;
